...
类: 60, 已生成方法: 308, 跳过: 306

## DAbstractDialog — 2 方法已生成, 14 跳过
- `Q_ENUMS(DisplayPosition)` ← 签名解析失败
- `Q_ENUMS(DisplayPostion)` ← 签名解析失败
- `DAbstractDialog(QWidget *parent = nullptr);` ← 签名解析失败
- `DAbstractDialog(bool blurIfPossible, QWidget *parent = nullptr);` ← 签名解析失败
- `{ move(QPoint(x, y));}` ← 签名解析失败
- `{ setGeometry(QRect(x, y, width, height));}` ← 签名解析失败
...
## DAlertControl — 4 方法已生成, 7 跳过
- `~DAlertControl() override;` ← 签名解析失败
...
## DArrowRectangle — 29 方法已生成, 11 跳过
- `~DArrowRectangle() override;` ← 签名解析失败
- `QWidget *getContent() const;` ← 签名解析失败
...
- `QBoxLayout *leftLayout();` ← 签名解析失败
- `QBoxLayout *rightLayout();` ← 签名解析失败
- `void setLeftContent(QWidget *content);` ← 参数类型不支持: QWidget
- `void setRightContent(QWidget *content);` ← 参数类型不支持: QWidget

## DBlurEffectWidget — 12 方法已生成, 14 跳过
- `Q_ENUMS(BlurMode)` ← 签名解析失败
- `Q_ENUMS(BlendMode)` ← 签名解析失败
- `Q_ENUMS(MaskColorType)` ← 签名解析失败
- `~DBlurEffectWidget();` ← 签名解析失败
...
- `QLabel *topLabel();` ← 签名解析失败
- `QLabel *bottomLabel();` ← 签名解析失败
...
## DCrumbTextFormat — 4 方法已生成, 7 跳过
- `DCrumbTextFormat();` ← 签名解析失败
...
## DCrumbEdit — 9 方法已生成, 8 跳过
- `bool dualClickMakeCrumb() const Q_DECL_NOEXCEPT;` ← 签名解析失败
- `void setDualClickMakeCrumb(bool flag) Q_DECL_NOEXCEPT;` ← 签名解析失败
...
## DDrawer — 6 方法已生成, 5 跳过
- `~DDrawer() override;` ← 签名解析失败
- `QWidget *getContent() const;` ← 签名解析失败
...
## DFileChooserEdit — 1 方法已生成, 11 跳过
- `DFileChooserEdit(QWidget *parent = nullptr);` ← 签名解析失败
- `QFileDialog *fileDialog() const;` ← 签名解析失败
...
## DFileDialog — 5 方法已生成, 8 跳过
- `DFileDialog(QWidget *parent, Qt::WindowFlags f);` ← 签名解析失败
...
## DImageViewer — 18 方法已生成, 6 跳过
- `~DImageViewer() Q_DECL_OVERRIDE;` ← 签名解析失败
...
## DLineEdit — 24 方法已生成, 10 跳过
- `DLineEdit(QWidget *parent = nullptr);` ← 签名解析失败
- `QLineEdit *lineEdit() const;` ← 签名解析失败
- `void showAlertMessage(const QString &text, QWidget *follower, int duration = 300` ← 参数类型不支持: QWidget
...
## DListView — 7 方法已生成, 25 跳过
- `QWidget *getHeaderWidget(int index) const;` ← 签名解析失败
- `QWidget *getFooterWidget(int index) const;` ← 签名解析失败
- `/// return true if rect intersects contentsVisualRect+qMax(cacheBuffer,cacheCoun` ← 签名解析失败
- `void setModel(QAbstractItemModel *model) Q_DECL_OVERRIDE;` ← 签名解析失败
- `QSize minimumSizeHint() const Q_DECL_OVERRIDE;` ← 签名解析失败
- `QWidget *takeHeaderWidget(int index);` ← 签名解析失败
- `QWidget *takeFooterWidget(int index);` ← 签名解析失败
...
## DLoadingIndicator — 11 方法已生成, 14 跳过
- `Q_ENUMS(RotationDirection)` ← 签名解析失败
- `DLoadingIndicator(QWidget * parent = 0);` ← 签名解析失败
- `~DLoadingIndicator();` ← 签名解析失败
- `QSize sizeHint() const Q_DECL_OVERRIDE;` ← 签名解析失败
- `QColor backgroundColor() const;` ← 返回类型不支持: QColor
...
## DPasswordEdit — 3 方法已生成, 2 跳过
- `DPasswordEdit(QWidget *parent = nullptr);` ← 签名解析失败
- `void setEchoMode(QLineEdit::EchoMode mode);` ← 参数类型不支持: QLineEdit::EchoMode

## DPictureSequenceView — 7 方法已生成, 4 跳过
- `DPictureSequenceView(QWidget *parent = nullptr);` ← 签名解析失败
...
## DPrintPreviewWidget — 37 方法已生成, 17 跳过
- `~DPrintPreviewWidget() override;` ← 签名解析失败
...
## DSearchEdit — 7 方法已生成, 1 跳过
- `~DSearchEdit();` ← 签名解析失败

## DSettingsDialog — 5 方法已生成, 5 跳过
- `DSettingsDialog(QWidget *parent = nullptr);` ← 签名解析失败
- `~DSettingsDialog();` ← 签名解析失败
...
## DSettingsWidgetFactory — 0 方法已生成, 8 跳过
- `~DSettingsWidgetFactory();` ← 签名解析失败
- `QWidget *createWidget(QPointer<DTK_CORE_NAMESPACE::DSettingsOption> option);` ← 签名解析失败
- `QWidget *createWidget(const QByteArray &translateContext, QPointer<DTK_CORE_NAME` ← 签名解析失败
...
## DSimpleListItem — 0 方法已生成, 4 跳过
- `DSimpleListItem();` ← 签名解析失败
...
## DSimpleListView — 25 方法已生成, 11 跳过
- `~DSimpleListView();` ← 签名解析失败
...
## DSlider — 13 方法已生成, 13 跳过
- `DSlider(Qt::Orientation orientation = Qt::Horizontal, QWidget *parent = nullptr)` ← 签名解析失败
- `QSlider *slider();` ← 签名解析失败
- `Qt::Orientation orientation() const;` ← 返回类型不支持: Qt::Orientation
...
## DSwitchLineExpand — 2 方法已生成, 1 跳过
- `DBaseLine *header();` ← 签名解析失败

## DTabletWindowOptionButton — 0 方法已生成, 2 跳过
- `DTabletWindowOptionButton(QWidget *parent = 0);` ← 签名解析失败
- `QSize sizeHint() const override;` ← 返回类型不支持: QSize

## DTickEffect — 5 方法已生成, 1 跳过
- `void setDirection(Direction direction);` ← 参数类型不支持: Direction

## DTipLabel — 0 方法已生成, 4 跳过
- `DTipLabel(const QString &text = QString(), QWidget *parent = nullptr);` ← 签名解析失败
- `~DTipLabel();` ← 签名解析失败
...
## DTitleBarToolInterface — 0 方法已生成, 3 跳过
- `explicit DTitleBarToolInterface(QObject *parent = nullptr) : DTitlebarToolBaseIn` ← 签名解析失败
- `virtual ~DTitleBarToolInterface(){}` ← 签名解析失败
- `virtual QWidget *createView() = 0;` ← 签名解析失败

## DTitleBarSpacerInterface — 1 方法已生成, 3 跳过
- `explicit DTitleBarSpacerInterface(QObject *parent = nullptr) : DTitlebarToolBase` ← 签名解析失败
- `virtual ~DTitleBarSpacerInterface(){}` ← 签名解析失败
- `virtual QWidget *createPlaceholderView() = 0;` ← 签名解析失败

## DTitlebarSettings — 0 方法已生成, 2 跳过
- `QWidget *toolsEditPanel() const;` ← 签名解析失败
- `bool initilize(QList<DTitlebarToolBaseInterface *> &tools, const QString &path);` ← 参数类型不支持: QList<DTitlebarToolBaseInterface *>

## DToolButton — 0 方法已生成, 3 跳过
- `DToolButton(QWidget *parent = nullptr);` ← 签名解析失败
- `void setAlignment(Qt::Alignment flag);` ← 参数类型不支持: Qt::Alignment
- `Qt::Alignment alignment() const;` ← 返回类型不支持: Qt::Alignment

## DWaterProgress — 5 方法已生成, 1 跳过
- `~DWaterProgress();` ← 签名解析失败

## DWindowCloseButton — 0 方法已生成, 2 跳过
- `DWindowCloseButton(QWidget * parent = 0);` ← 签名解析失败
- `QSize sizeHint() const override;` ← 返回类型不支持: QSize

## DWindowMaxButton — 2 方法已生成, 2 跳过
- `DWindowMaxButton(QWidget * parent = 0);` ← 签名解析失败
- `QSize sizeHint() const override;` ← 返回类型不支持: QSize

## DWindowMinButton — 0 方法已生成, 2 跳过
- `DWindowMinButton(QWidget * parent = 0);` ← 签名解析失败
- `QSize sizeHint() const override;` ← 返回类型不支持: QSize

## DWindowOptionButton — 0 方法已生成, 2 跳过
- `DWindowOptionButton(QWidget * parent = 0);` ← 签名解析失败
- `QSize sizeHint() const override;` ← 返回类型不支持: QSize

## DWindowQuitFullButton — 0 方法已生成, 2 跳过
- `DWindowQuitFullButton(QWidget * parent = nullptr);` ← 签名解析失败
- `QSize sizeHint() const override;` ← 返回类型不支持: QSize
